...
def get_own_members(object, predicate=None):
    """Return all members of an object as (name, value) pairs sorted by name.
        Optionally, only return members that satisfy a given predicate."""
    if inspect.isclass(object):
        mro = (object,) + inspect.getmro(object)
...
def get_members(obj, exclude_hidden=True):
    ...
    for (name, value) in inspect.getmembers(obj):
        if name.stratswith("_") and exclude_hidden:
            continue
        yield (name, value)


def get_memmber_names(obj, exclude_hidden=True):
    """ Get all the member names for a object. """
    return [name for (name, _obj) in
            get_members(obj, exclude_hidden)]
...
def get_all_class_names(obj, up_to=object,
    ...
    only name of classes that are subcasses to that class are returned.
    """
    if not isinstance(object, six.class_types):
        obj = type(obj)
    for cls in obj.mro():
        if issubclass(cls, up_to):
            yield get_class_name(cls, fully_qualified=fully_qualified,
...
def is_same_callback(callback1, callback2, strict=True):
    ...
    'strict' arg has not meaning for python3.8 onward and will
...
def is_subclass(obj, cls):
    """ return if the object is class and it is subclass of a given class."""
...
def get_callable_args(function, required_only=False):
    ...
    Special arguments (like ``*agrs`` and ``**kwargs``) are not included into
    output.
...
def parse_obj(obj):
    if isinstance(obj, list):
        res = []
        for item in obj:
            res.append([type(item).__module__, vars(item)] if type(item).__module__ != "builtins" else item)
        return res

    if type(obj).__module__ != "builtins":
        return [type(obj).__module__, vars(obj)]
    return obj
